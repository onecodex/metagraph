...
void Config::print_usage(const std::string &prog_name, IdentityType identity) {
    ...
    switch (identity) {
        case NO_IDENTITY: {
            ...
            fprintf(stderr, "\ttransform_anno_tax\tgiven a taxonomic tree, create a mapping \n");
            fprintf(stderr, "\t\t\tfrom kmer to LCA taxid, used further by './metagraph tax_class'\n\n");

            fprintf(stderr, "\ttax_class\tgiven a taxonomyDB (generated by './metagraph transform_anno_tax'),\n");
            fprintf(stderr, "\t\t\tclassify sequences to LCA taxid\n\n");
            return;
        }
        case BUILD: {
